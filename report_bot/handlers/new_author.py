...
async def new_author(message, state):
    await message.answer("Введи имя и фамилию автора, ник и то, как помечаем его в таблице. "
...
async def add_author_to_db(message, state):
    ...
    try:
        ...
        await message.answer(f"{nickname} добавлен!\n\n"
                            f"Добавить еще одного — /new_author\n\n"
                            f"Посмотреть всех — /authors")
    except:
        pass
        await message.answer(f"Что-то не получилось. \n"
