...
async def check_text_in_turgenev(text):
        ...
        async with session.post(url, data=data) as response:
            ...
            if response.status == 200:
                try:
                    ...
                    raise ValueError(f"Ошибка декодирования JSON: {response_text}")
                ...
                if details:
                    ...
                    msg = "Детали отсутствуют в ответе."

            else:
                msg = f"Ошибка {response.status}: {response_text}"
